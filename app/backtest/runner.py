...
class _DayContext:
    ...
    def __init__(self, store: BarStore, day: date, cfg: BacktestConfig, screener_top: int):
        ...
        self.daily_history = {
            symbol: store.trailing_daily_bars(symbol, day, 20)
            for symbol in list(self.universe.slices)
        }
        times: set[datetime] = set()
        for day_slice in self.universe.slices.values():
            times.update(day_slice.times)
    ...
    def bar_at(self, symbol: str, moment: datetime) -> dict | None:
        ...
        if not day_slice:
            return None
        index = day_slice.index_at(moment)
        if index < 0 or day_slice.times[index] != moment:
            return None
...
def _prefilter(symbols: list[str], change_map: dict[str, float], ctx: _DayContext,
               moment: datetime, cfg_settings) -> list[str]:
    """Drop symbols `build_candidate` would reject outright on price or day-change.

    Both checks are unconditional rejections upstream, so removing them cannot
    change which candidates become eligible — it only trims diagnostic rows.
    """
    kept = []
    for symbol in symbols:
        price = ctx.price_at(symbol, moment)
        if price is None or not (cfg_settings.min_price <= price <= cfg_settings.max_price):
            continue
        change = change_map.get(symbol, 0.0)
        if not (cfg_settings.min_change_pct <= change <= cfg_settings.max_change_pct):
            continue
        kept.append(symbol)
    return kept
    ...
    with override_settings(cfg_settings):
        for day in days:
            ...
            for moment in ctx.timeline:
                # 1. Advance held/working symbols through this bar.
                for symbol in list(broker.exposure_symbols()):
                    bar = ctx.bar_at(symbol, moment)
                    if bar is not None:
                        broker.process_bar(symbol, bar, moment, cfg.use_true_initial_risk)

                marks = {s: (ctx.price_at(s, moment) or p.entry_price) for s, p in broker.positions.items()}
                regime = ctx.regime_at(moment)

                # 2. Manager exit rules on everything still open.
                for symbol in list(broker.positions):
                    position = broker.positions[symbol]
                    if position.entry_time >= moment:
                        continue  # managed from the bar after the fill
                    current = marks.get(symbol)
                    if not current:
                        continue
                    decision = evaluate_exit(position.state, current, ctx.window_at(symbol, moment),
                                             regime, moment, cfg_settings)
                    if decision:
                        broker.close(symbol, current, moment, decision.reason, decision.meta)
                        marks.pop(symbol, None)

                equity = broker.equity(marks)
                equity_curve.append(EquityPoint(moment, equity))
                ...
                bars_map = {}
                for symbol in symbols:
                    day_slice = ctx.universe.slices[symbol]
                    index = day_slice.index_at(moment)
                    snapshots[symbol] = build_snapshot(day_slice, index, cfg.execution)
                    bars_map[symbol] = day_slice.window(index)

                fraction = session_fraction(moment)
                rows = assemble_candidates(symbols, change_map, active_rank, snapshots, bars_map,
                                           ctx.daily_history, regime, fraction)
                enriched = [enrich_candidate(r) for r in rows]
                for row in enriched:
                    for reason in row.get("reject_reasons", []):
                        rejected_reasons[reason] = rejected_reasons.get(reason, 0) + 1

                eligible = [r for r in enriched
                ...
                for candidate in eligible[:MAX_CANDIDATES_PER_SCAN]:
                    ...
                    if _cooldown_active(cooldowns, symbol, moment):
                        continue
                    if arming.arm_or_confirm(candidate):
                        if _submit(broker, candidate, equity, moment, cfg, cfg_settings, cooldowns):
                            orders_placed += 1
                        break

            # 6. Session close: flatten anything still open.
            ...
            if progress:
                progress(day, len(broker.trades))

    return BacktestResult(
        trades=broker.trades, equity_curve=equity_curve, daily_equity=daily_equity,
...
def _submit(broker: SimulatedBroker, candidate: dict, equity: float, moment: datetime,
    ...
    """Mirror of engine.maybe_place_paper_order's guard chain."""
    symbol = candidate["symbol"].upper()
    ...
    if risk_pct > cfg_settings.max_stop_pct:
        return False
    # Live counts filled broker positions only, so working orders do not occupy a slot.
    if len(broker.positions) >= cfg_settings.max_open_positions:
        return False
    ...
    broker.place(PendingOrder(
        symbol=symbol, limit=entry, stop=stop, target=target, qty=qty, placed_at=moment,
        meta={"score": candidate.get("score"), "intel_score": candidate.get("intel_score"),
              "grade": candidate.get("grade"), "rvol": candidate.get("rvol"),
              "change_pct": candidate.get("change_pct"), "risk_pct": round(risk_pct, 2),
              "entry_hour": moment.astimezone(NY).hour},
    ))

...
def build_candidate(symbol: str, change_pct: float, volume_rank: int, snapshot: dict, bars: list[dict]) -> Candidate:
    ...
    vwap = _vwap(bars)
    ema9 = _ema(closes[-60:], 9) if closes else 0.0
    ema20 = _ema(closes[-80:], 20) if closes else 0.0
    atr = _atr(bars, 14)
    ...
    if len(bars) >= 1:
        ...
        c = _num(bars[-1].get("c"))
        if o > 0:
            one_bar_move = abs(c / o - 1) * 100

    recent = bars[-10:] if len(bars) >= 10 else bars
    support = max(vwap, ema20) if vwap > 0 and ema20 > 0 else max(vwap, ema20)
    pullback_seen = False
    if support > 0 and recent:
        pullback_seen = any(abs(_num(b.get("l")) / support - 1) <= 0.012 for b in recent[:-1])
    ...
    if len(bars) < settings.min_bars:
        reject.append("Insufficient intraday history")
    if not (settings.min_price <= price <= settings.max_price):
        reject.append("Price outside allowed range")
    if not (settings.min_change_pct <= change_pct <= settings.max_change_pct):
        reject.append("Move too weak or already overextended")
    if spread_pct > settings.max_spread_pct:
        reject.append("Spread too wide")
    if vwap_ext > settings.max_vwap_extension_pct:
        reject.append("FOMO block: too far above VWAP")
    if ema20_ext > settings.max_ema20_extension_pct:
        reject.append("FOMO block: too far above EMA20")
    if one_bar_move > settings.max_one_bar_move_pct:
        reject.append("FOMO block: current 1m candle too large")
    if not pullback_seen:
        reject.append("No controlled pullback yet")
    if not reclaim_confirmed:
        reject.append("No VWAP/EMA reclaim confirmation")
    if not volume_confirmed:
        reject.append("Reclaim volume not confirmed")

    # Quality score rewards a controlled setup rather than raw % gain.
    score = 0.0
    if len(bars) >= settings.min_bars: score += 10
    if spread_pct <= 0.20: score += 15
    elif spread_pct <= settings.max_spread_pct: score += 10
    if 0 <= vwap_ext <= 1.2: score += 20
    elif 0 <= vwap_ext <= settings.max_vwap_extension_pct: score += 12
    if 0 <= ema20_ext <= 1.0: score += 15
    elif 0 <= ema20_ext <= settings.max_ema20_extension_pct: score += 8
    if pullback_seen: score += 15
    if reclaim_confirmed: score += 15
    if volume_confirmed: score += 10
    if volume_rank <= 10: score += 5
    if settings.min_change_pct <= change_pct <= 20: score += 5
    ...
    # Structure/ATR stop. Use the lowest recent swing with an ATR buffer.
    ...
    if risk_pct < settings.min_stop_pct:
        stop = price * (1 - settings.min_stop_pct / 100)
        risk_pct = settings.min_stop_pct
    if risk_pct > settings.max_stop_pct:
        reject.append("Required stop is too wide")

    entry = round(price, 2)
    stop = round(stop, 2)
    risk_dollars = max(entry - stop, 0)
    ...
    return Candidate(
        ...
        volume_rank=volume_rank, spread_pct=round(spread_pct, 3), score=score,
        eligible=eligible, setup=setup, reasons=reasons, reject_reasons=reject,
        vwap=round(vwap, 4), ema9=round(ema9, 4), ema20=round(ema20, 4), atr=round(atr, 4),
        vwap_extension_pct=round(vwap_ext, 2), one_bar_move_pct=round(one_bar_move, 2),
        pullback_seen=pullback_seen, reclaim_confirmed=reclaim_confirmed,
        volume_confirmed=volume_confirmed, entry=entry, stop=stop, target=target,
...
def position_size(equity: float, entry: float, stop: float) -> int:
    ...
    # Max 15% account concentration per position.
    cap = int((equity * 0.15) / entry)

...
def evaluate_exit(state: PositionState, current: float, bars: list[dict], regime: dict,
    ...
    # Runner mode: once a trade reaches meaningful expansion, stop thinking in
    # terms of a fixed target. Protect a fraction of the best R reached so a
    # +5R/+10R/+20R move can continue while still preventing a full round trip.
    if getattr(cfg, "runner_mode", True) and r_high >= getattr(cfg, "runner_activate_r", 2.0):
        ...
        if r_now <= floor:
            return ExitDecision("runner_trailing_exit", {"r_high": round(r_high,2), "r_now": round(r_now,2), "locked_floor_r": round(floor,2)})
